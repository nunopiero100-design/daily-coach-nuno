...
def detect_workout_family_for_actions(name: str, description: str = "") -> str:
    """
    Classifica o treino planeado para gerar alternativas curtas coerentes.
    Evita que dias VO2 recebam alternativas genéricas de tempo/SS.
    """
    txt = f"{name or ''} {description or ''}".lower()

    if "vo2" in txt or "vo₂" in txt or "vo2max" in txt:
        return "vo2"

    if "over-under" in txt or "over under" in txt or "over/under" in txt:
        return "over_under"

    if "threshold" in txt or "limiar" in txt or "ftp" in txt or "tte" in txt:
        return "threshold"

    if "sweet spot" in txt or "sweetspot" in txt or " ss " in f" {txt} ":
        return "sweet_spot"

    if "social" in txt or "grupo" in txt or "group" in txt or "cap" in txt:
        return "social"

    if "z2" in txt or "zone 2" in txt or "endurance" in txt or "recovery" in txt:
        return "z2"

    return "generic"


def extract_interval_minutes_for_actions(name: str, description: str = "", default: int = 3) -> int:
    """
    Extrai a duração dos intervalos de padrões como 4x3, 4 x 3, 5x4, 4 x 5 min.
    """
    txt = f"{name or ''} {description or ''}".lower()
...
def vo2_short_action_lines(name: str, description: str = "", status: str = "VERDE") -> tuple[str, str, str, str, str]:
    """
    Alternativas curtas específicas para dias VO2.
    Mantém o estímulo VO2, mas corta volume em vez de trocar por tempo/SS.
    """
    rep_min = extract_interval_minutes_for_actions(name, description, default=3)
    status = str(status or "").upper()

    if rep_min <= 3:
        if status == "AMARELO":
            a60 = "Se só tiveres 60 min: 10–15 min aquecer, 2–3x3 min VO2 controlado @105–108% com 3 min muito fácil, resto Z2 fácil e 5 min arrefecer."
            a45 = "Se só tiveres 45 min: 10 min aquecer, 2x3 min VO2 controlado @105–108% com 3 min muito fácil, resto Z2 fácil e 5 min arrefecer; se estiver pesado, fica só em Z2."
            indoor = "Se for indoor/rolo: cadência alta 95–105 rpm, ventoinha forte, recuperações mesmo fáceis; se a cadência cair, cortar uma repetição."
        else:
            a60 = "Se só tiveres 60 min: 10–15 min aquecer, 3x3 min VO2 @108–110% com 3 min muito fácil, resto Z2 fácil e 5 min arrefecer. Sem sprint final."
            a45 = "Se só tiveres 45 min: 10 min aquecer, 2x3 min VO2 @108–110% com 3 min muito fácil, resto Z2 fácil e 5 min arrefecer. Não compensar com watts extra."
            indoor = "Se for indoor/rolo: fazer em ERG com cadência alta 95–105 rpm. Recuperações mesmo fáceis. Se a cadência cair ou o ERG prender, reduzir 2–3% ou cortar uma repetição."
    elif rep_min == 4:
        if status == "AMARELO":
            a60 = "Se só tiveres 60 min: 10–15 min aquecer, 2–3x4 min VO2 controlado @105–108% com 4 min muito fácil, resto Z2 fácil e 5 min arrefecer."
            a45 = "Se só tiveres 45 min: 10 min aquecer, 2x4 min VO2 controlado @105–108% com 4 min muito fácil, resto Z2 fácil e 5 min arrefecer; se estiver pesado, Z2."
            indoor = "Se for indoor/rolo: cadência alta 95–105 rpm, recuperações muito fáceis. Se a 2.ª repetição já parecer limite, fazer só 2–3 reps."
        else:
            a60 = "Se só tiveres 60 min: 10–15 min aquecer, 3x4 min VO2 @108–110% com 4 min muito fácil, resto Z2 fácil e 5 min arrefecer. Qualidade > quantidade."
            a45 = "Se só tiveres 45 min: 10 min aquecer, 2x4 min VO2 @108–110% com 4 min muito fácil, resto Z2 fácil e 5 min arrefecer. Não compensar com watts extra."
            indoor = "Se for indoor/rolo: cadência alta 95–105 rpm, ventoinha forte, sem começar acima do alvo. Se a 2.ª repetição já parecer limite, cortar uma repetição."
    else:
        if status == "AMARELO":
            a60 = "Se só tiveres 60 min: 10–15 min aquecer, 2–3x4 min VO2 controlado @105–108% com 4 min muito fácil, resto Z2 fácil e 5 min arrefecer. Encurtar reps longas antes de subir watts."
            a45 = "Se só tiveres 45 min: 10 min aquecer, 2x4 min VO2 controlado @105–108% com 4 min muito fácil, resto Z2 fácil e 5 min arrefecer."
            indoor = "Se for indoor/rolo: VO2 longo em ERG só com cadência alta. Se prender as pernas, encurtar repetições antes de subir watts."
        else:
            a60 = "Se só tiveres 60 min: 10–15 min aquecer, 3x4 min VO2 @108–110% com 4 min muito fácil, resto Z2 fácil e 5 min arrefecer. Reduzir reps longas para 4 min por segurança."
            a45 = "Se só tiveres 45 min: 10 min aquecer, 2x4 min VO2 @108–110% com 4 min muito fácil, resto Z2 fácil e 5 min arrefecer."
            indoor = "Se for indoor/rolo: cadência alta 95–105 rpm. Se o VO2 longo prender as pernas, encurtar repetições antes de subir watts."

    weekend_alt = "Se for sábado/domingo e não der para sair: 60–90 min indoor com Z2 dominante; se mantiveres VO2, fazer só 2–3 reps curtas e controladas."
    weekend_weather = f"Se chover/condições forem más: faz {name or 'o treino VO2 planeado'} indoor/rolo com versão curta se necessário; boa ventilação e sem watts extra."
...
def normalize_practical_actions(decision, context):
    ...
    if planned:
        ...
        planned_description = str(planned[0].get("description") or "")
        planned_load = planned[0].get("load") or 0
        planned_hours = planned[0].get("hours") or 0

    lname = name.lower()
    workout_family = detect_workout_family_for_actions(name, planned_description)
    is_vo2_workout = workout_family == "vo2"
    easy_terms = [
        "zone 2", "z2", "endurance", "recovery", "recuper", "easy", "facil", "fácil"
    ]
    quality_terms = [
    ...
    if status == "VERMELHO":
        ...
        if is_easy:
            ...
            if is_vo2_workout:
                a60, a45, indoor, weekend_alt, weekend_weather = vo2_short_action_lines(name, planned_description, status="AMARELO")
            else:
        ...
        if is_easy:
            ...
            if is_vo2_workout:
                a60, a45, indoor, weekend_alt, weekend_weather = vo2_short_action_lines(name, planned_description, status="VERDE")
            else:
