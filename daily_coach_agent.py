...
def calc_coach_recovery_score(context):
    """
    Score numerico local e auditavel. Nao depende da OpenAI.
    """
    ...
    missing = [k for k in ("sleep_hours", "hrv", "resting_hr") if tm.get(k) is None]
    if missing:
        return {"score": None, "label": "DADOS INCOMPLETOS", "penalties": ["faltam: " + ", ".join(missing)]}

    score = 100.0
    penalties = []

    sleep_hours = tm.get("sleep_hours")
    sleep_score_v = tm.get("sleep_score")
    if sleep_hours is not None:
        if sleep_hours < 5.75:
            score -= 22; penalties.append("sono muito baixo")
        elif sleep_hours < 6.5:
            score -= 14; penalties.append("sono baixo")
        elif sleep_hours < 7.0:
            score -= 7; penalties.append("sono curto")
    if sleep_score_v is not None:
        if sleep_score_v < 65:
            score -= 10; penalties.append("sleep score baixo")
        elif sleep_score_v < 80:
            score -= 5; penalties.append("sleep score moderado")

    hrv_now = tm.get("hrv")
    hrv_base = bl.get("hrv")
    if hrv_now is not None and hrv_base:
        diff = (hrv_now - hrv_base) / hrv_base * 100
        if diff <= -15:
            score -= 24; penalties.append("HRV muito baixa")
        elif diff <= -10:
            score -= 15; penalties.append("HRV baixa")
        elif diff <= -5:
            score -= 7; penalties.append("HRV ligeiramente baixa")
        elif diff >= 8:
            score += 3
    ...
    if rhr_now is not None and rhr_base:
        ...
        if diff >= 7:
            score -= 20; penalties.append("RHR muito alta")
        elif diff >= 4:
            score -= 12; penalties.append("RHR alta")
        elif diff >= 2:
            score -= 5; penalties.append("RHR ligeiramente alta")
        elif diff <= -3:
            score += 2

    fm = tm.get("readiness_form")
    if fm is None:
        fm = tm.get("form")
    if fm is not None:
        if fm < -30:
            score -= 18; penalties.append("Form muito negativa")
        elif fm < -22:
            score -= 12; penalties.append("Form baixa")
        elif fm < -15:
            score -= 7; penalties.append("Form moderadamente baixa")
        elif fm <= -10:
            score -= 3; penalties.append("Form ligeiramente negativa")

    recent_load = recent.get("load") or 0
    if recent_load >= 450:
        score -= 10; penalties.append("carga 3d muito alta")
    elif recent_load >= 300:
        score -= 6; penalties.append("carga 3d alta")
    elif recent_load >= 240:
        score -= 3; penalties.append("carga 3d moderada")

    y_status = str(y.get("status") or "").upper()
    y_ratio = y.get("ratio")
    if "MAIS DURO" in y_status:
        score -= 6; penalties.append("ontem acima do plano")
    elif y_ratio is not None and y_ratio >= 1.15:
        score -= 4; penalties.append("ontem ligeiramente acima")

    score = int(round(clamp(score, 0, 100)))
    return {"score": score, "label": coach_score_label(score), "penalties": penalties}


def first_action(actions, prefix="Plano normal"):
    for a in actions or []:
        if str(a).lower().startswith(prefix.lower()):
            return str(a)
...
def clean_action_line(action):
    ...
    s = re.sub(r"^Plano normal ajustado:\s*", "", s, flags=re.I)
    s = re.sub(r"^Plano normal:\s*", "", s, flags=re.I)
...
Responde APENAS em JSON válido:
{
  ...
  "decision_text": "resumo curto em português",
  "reasons": ["..."],
  "actions": ["..."],
...
""".strip()
    ...
    # Structured Outputs: force valid JSON so the agent does not fall back
    # because of malformed model text.
    coach_schema = {
        "type": "object",
        ...
        "properties": {
            ...
            "should_modify_intervals": {"type": "boolean"},
        },
        "required": ["status", "decision_text", "reasons", "actions", "should_modify_intervals"],
    }

    payload = {
        "model": model,
        "instructions": instructions,
        "input": "Dados do dia em JSON:\n" + json.dumps(context, ensure_ascii=False, indent=2),
    ...
    try:
        ...
        if not r.ok:
            return None, f"OpenAI API falhou HTTP {r.status_code}: {r.text[:800]}"
        data = r.json()
        ...
        if not text:
            ...
            text = "\n".join(parts).strip()
        if not text:
            return None, "OpenAI respondeu sem texto; usei regras heurísticas."
        try:
            obj = json.loads(text)
            ...
            obj = json.loads(raw)
        if obj.get("status") not in ("VERDE", "AMARELO", "VERMELHO"):
...
def main():
    ...
    score_info = calc_coach_recovery_score(context)
    context["coach_recovery_score"] = score_info

    lines = []
    lines.append("=" * 72)
    lines.append(f"DAILY COACH AGENT — {target.isoformat()} — {COACH_SYSTEM_VERSION}")
    lines.append("=" * 72)
    ...
    if context["planned_events_today"]:
        for e in context["planned_events_today"]:
            ext = "OK" if e.get("external_id") else "n/d"
            lines.append(f"{e.get('name')} | load={fmt(e.get('load'),0)} | duração={fmt_h(e.get('hours'))} | external_id={ext}")
    else:
        lines.append("Sem treino planeado encontrado.")
    if context["completed_activities_today"]:
        done_txt = " + ".join(
            f"{a.get('name') or a.get('type') or 'atividade'} ({fmt(a.get('load'),0)} TSS | {fmt_h(a.get('hours'))})"
            for a in context["completed_activities_today"]
        )
        lines.append(f"Já feito hoje: {done_txt}")
    lines.append("")
    lines.append("ONTEM")
    yc = context["yesterday"]["compliance"]
    lines.append(f"{yc['status']} — {fmt(yc['done_load'],0)} / {fmt(yc['planned_load'],0)} TSS | {fmt_h(yc['done_hours'])} / {fmt_h(yc['planned_hours'])}")
    lines.append("")
    tm = context["today_metrics"]
    bl = context["baseline_14d"]
    lines.append("READINESS")
    if score_info.get("score") is None:
        lines.append(f"Coach Recovery Score: n/d | {score_info.get('label')}")
    else:
        lines.append(f"Coach Recovery Score: {score_info.get('score')}/100 | {score_info.get('label')}")
    ...
    decision_line = decision.get("decision_text") or "n/d"
    lines.append(f"{decision.get('status')} — {decision_line}")
    if decision.get("status") in ("AMARELO", "VERMELHO", "JÁ FEITO"):
        reasons = list(decision.get("reasons", []))[:3]
        if reasons:
    ...
    if applied:
        ...
        if str(apply_error).startswith("AUTO_APPLY"):
    ...
    Path("daily_agent_report.json").write_text(json.dumps({
        "context": context,
        "coach_recovery_score": score_info,
