...
def summarize_prior_week(events, activities, week_start):
    ...
    done = [a for a in activities if parse_item_date(a) and start <= parse_item_date(a) <= end and ((metric_hours(a) or 0) > 0.15 or (metric_load(a) or 0) > 5)]
    ...
    if p_load > 50 and d_load > p_load * 1.25:
    ...
    return {"start": start.isoformat(), "end": end.isoformat(), "status": status, "planned_load": p_load, "done_load": d_load, "planned_hours": p_h, "done_hours": d_h, "planned_count": len(planned), "done_count": len(done)}
...
def decide_adjustment(macro_week, prior, latest_w):
    ...
    ratio = prior["done_load"] / prior["planned_load"] if prior["planned_load"] > 50 else None
    fm = form(latest_w) if latest_w else None
    sh = sleep_h(latest_w) if latest_w else None
    if ratio is not None and ratio > 1.25:
        reasons.append(f"Semana anterior acima do planeado: {fmt(prior['done_load'],0)} vs {fmt(prior['planned_load'],0)} TSS.")
    if fm is not None and fm < -22:
        reasons.append(f"Form baixa: {fmt(fm,0)}.")
    if sh is not None and sh < 6:
        reasons.append(f"Sono recente baixo: {fmt_h(sh)}.")
    if (ratio is not None and ratio > 1.35) or (fm is not None and fm < -28):
        return "recovery", reasons or ["Sinais fortes de fadiga."]
    if (ratio is not None and ratio > 1.15) or (fm is not None and fm < -18):
        return "reduced", reasons or ["Sinais moderados de fadiga."]
...
def main():
    ...
    if args.target_week_start:
        ...
        week_start = monday_of(dt.datetime.now(ZoneInfo(TIMEZONE)).date())
    ...
    for w in wellness:
        ...
        if d:
            by_day[d] = w
    latest_day = max(by_day.keys()) if by_day else None
